...
MD_SUFFIX="${CLAUDISH_MD_SUFFIX:-clair}"
...
MARKER="<!-- claudish-to-french:rewritten -->"
LOG_ROOT="${TMPDIR:-/tmp}/claudish-to-french"
...
if [ "$STUB" = "1" ]; then
  ...
  sys="Tu réécris la prose Markdown en français clair et simple. Garde chaque fait, nom, nombre, lien et chemin de fichier. Garde toute la structure Markdown: titres, listes, tableaux, liens. Ne modifie PAS les blocs de code ni le frontmatter YAML; reproduis-les exactement. Phrases courtes, mots du quotidien. Pas de tiret cadratin ni demi-cadratin. Sortie UNIQUEMENT le Markdown réécrit, sans préambule, sans étiquette, sans commentaire."
...
if [ -z "$rewrite" ]; then
  ...
  if [ "$NOTICE" = "1" ] && [ ! -e "$notified" ]; then
    why=""
    if [ "$curl_rc" = "28" ]; then
      why="réécriture de $(basename "$file") expirée après ${LLM_TIMEOUT}s - modèle trop lent. Augmente CLAUDISH_MD_TIMEOUT (et le timeout PostToolUse dans hooks.json), ou mets un CLAUDISH_MODEL plus petit. Fichier inchangé."
    elif [ "$curl_rc" != "0" ]; then
      why="impossible de joindre ollama à $OLLAMA - réécriture Markdown ignorée, fichier inchangé. Lance \`ollama serve\`."
    elif printf '%s' "${err:-}" | grep -qi 'not found'; then
      why="modèle ollama '$MODEL' introuvable - réécriture Markdown ignorée, fichier inchangé. \`ollama pull $MODEL\` ou change CLAUDISH_MODEL."
    elif [ -n "${err:-}" ]; then
      why="erreur ollama pendant la réécriture Markdown: $err. Fichier inchangé."
    fi
    if [ -n "$why" ]; then
      : > "$notified" 2>/dev/null || true
      jq -n --arg m "claudish-to-french: $why (une fois par session; CLAUDISH_NOTICE=0 pour couper)" \
