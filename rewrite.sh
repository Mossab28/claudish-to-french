...
BUF_ROOT="${TMPDIR:-/tmp}/claudish-to-french"
SEP=$'\n\n────────────────────────\n💬 En français clair:\n\n'
...
if [ "$STUB" = "1" ]; then
  ...
  sys="Tu réécris le message de l'assistant en français clair et simple. Garde chaque fait, nom, nombre et chemin de fichier. Phrases courtes, mots du quotidien. Ne touche pas aux blocs de code délimités par des \`\`\`. Sortie UNIQUEMENT le message réécrit, sans préambule, sans étiquette, sans commentaire. N'utilise pas de tiret cadratin ni demi-cadratin."
  ...
  if [ -n "$userq" ]; then
    sys="$sys"$'\n\n'"Pour contexte, l'utilisateur a demandé à l'assistant: \"$userq\". Utilise ça seulement pour comprendre le message. Ne réécris PAS, ne réponds PAS et ne répète PAS la question de l'utilisateur - réécris uniquement le message de l'assistant qui suit."
...
if [ -z "$rewrite" ]; then
  ...
  if [ "$NOTICE" = "1" ] && [ ! -e "$notified" ] && { [ "$curl_rc" != "0" ] || [ -n "${err:-}" ]; }; then
    ...
    if [ "$curl_rc" = "28" ]; then
      why="réécriture expirée après ${LLM_TIMEOUT}s (modèle trop lent) - augmente CLAUDISH_TIMEOUT ou mets un CLAUDISH_MODEL plus petit"
    elif [ "$curl_rc" != "0" ]; then
      why="impossible de joindre ollama à $OLLAMA - lance \`ollama serve\` (voir le README)"
    elif printf '%s' "${err:-}" | grep -qi 'not found'; then
      why="modèle ollama '$MODEL' introuvable - \`ollama pull $MODEL\`, ou change CLAUDISH_MODEL"
    else
      why="erreur ollama: ${err:-inconnu}"
    fi
    note=$'\n\n────────────────────────\n'"⚠️ claudish-to-french: $why. Texte original de Claude affiché tel quel. Une fois par session; CLAUDISH_NOTICE=0 pour couper."
